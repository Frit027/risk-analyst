from app import Solution, Category, Condition, Requirement, Deal, Document
...
class DatabaseFiller:
    ...
    def fill_category(db):
        with open('static/csv/categories.csv', encoding='utf-8') as f:
            for name in f:
                db.session.add(Category(name=name.rstrip()))

    @staticmethod
    def fill_requirements(db):
        with open('static/csv/requirements.csv', encoding='utf-8') as f:
            for data in csv.reader(f):
                db.session.add(Requirement(name=data[0]))

    @staticmethod
    def fill_conditions(db):
        with open('static/csv/conditions.csv', encoding='utf-8') as f:
            for data in csv.reader(f):
    ...
    def fill(db):
        ...
        with open('static/csv/data.csv', encoding='utf-8') as f:
            ...
            for i, row in enumerate(reader):
                ...
                # print(f"{i}:{'': <2}{deal_id: <18}{doc_id: <33}{doc_status: <20}{deal_status: <24}{cond: <24}{req: <24}",
                #       file=sys.stdout)
